...
const CardEditorModal: React.FC<{
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-gradient-to-b from-[#0e1a32] to-[#0a1524] border border-[var(--grad-1)]/20 rounded-xl shadow-2xl w-full max-w-2xl h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <main className="p-4 space-y-4 overflow-y-auto flex-grow flex flex-col">
                    <div className="flex-1 flex flex-col">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            <Button variant="glass" className="text-xs" onClick={() => qImgRef.current?.click()}>Add Image</Button>
                    ...
                    <div className="flex-1 flex flex-col">
                        ...
                        <RichTextEditor value={data.a} onChange={content => setData(p => ({ ...p, a: content }))} />
                         <div className="flex items-center gap-2 mt-2">
                            <Button variant="glass" className="text-xs" onClick={() => aImgRef.current?.click()}>Add Image</Button>
...
const StudySessionView: React.FC<{ deck: Deck, onEnd: (updatedCards: Flashcard[]) => void }> = ({ deck, onEnd }) => {
    ...
    if (studyQueue.length === 0) {
        return (
            <div className="fixed inset-0 bg-bg z-50 flex flex-col items-center justify-center p-4 text-center">
    ...
    return (
        <div className="fixed inset-0 bg-bg z-50 flex flex-col items-center justify-center p-4">
    ...
    return (
        <>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                {decks.length === 0 ? <p className="text-gray-400">No decks created yet.</p> :
                    decks.map((deck) => {
                            ...
                            <div key={deck.name} className="bg-black/20 p-3 rounded-lg">
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button variant="glass" onClick={() => setEditingDeck(isEditingThis ? null : deck)}>{isEditingThis ? 'Close' : 'Manage'}</Button>
                                        <Button variant="glass" className="text-red-400" onClick={() => deleteDeck(deck.name)}>Del</Button>
                                    </div>
                                </div>
                                {isEditingThis && (
                                        ...
                                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                                            {deck.cards.map(card => (
                                                <div key={card.id} className="flex justify-between items-center p-2 bg-white/5 rounded-md">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <Button variant="glass" className="text-xs !p-1" onClick={() => setEditingCard(card)}>Edit</Button>
                                                        <Button variant="glass" className="text-xs !p-1" onClick={() => handleDeleteCard(card.id)}>Del</Button>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
