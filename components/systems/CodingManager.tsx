...
// These functions yield the state of the array and metadata at each step
...
const AlgorithmVisualizer: React.FC<{ initialAlgo?: string }> = ({ initialAlgo }) => {
    const [algoType, setAlgoType] = useState(initialAlgo || 'bubble');
    const [arraySize, setArraySize] = useState(15);
    const [speed, setSpeed] = useState(100); // ms
    const [isPlaying, setIsPlaying] = useState(false);
    
    // visualState tracks what to render. 
    // IMPORTANT: We also use a ref to track the latest array state for the generator initialization to avoid stale closures.
    const [visualState, setVisualState] = useState<VisualizerStep>({ arr: [], indices: [], type: 'idle', message: 'Ready' });
    const arrRef = useRef<number[]>([]); 
    
    // Persist generator across renders
    ...
    const initGenerator = () => {
        // Always initialize generator with the LATEST known array state
    ...
    const step = useCallback(() => {
        ...
        if (next && !next.done) {
            ...
            arrRef.current = next.value.arr; // Keep ref in sync with visual state
        } else {
            setIsPlaying(false);
            if (timerRef.current) clearInterval(timerRef.current);
        }
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="flex flex-wrap gap-4 mb-4 p-4 bg-black/20 rounded-lg border border-white/10">
            ...
            <div className="flex-grow bg-black/40 rounded-lg border border-white/10 relative flex items-end justify-center gap-1 p-4 overflow-hidden">
                 <div className="absolute top-4 left-4 text-sm font-mono text-green-400 bg-black/50 p-2 rounded border border-green-500/30">
                    {visualState.message}
                </div>
                {visualState.arr.map((val, idx) => (
                    <div 
                        key={idx} 
                        className={`w-full max-w-[30px] rounded-t-sm transition-all duration-100 ${getBarColor(idx)}`}
                        style={{ height: `${(val / 60) * 100}%` }}
                    >
                        {arraySize <= 20 && <span className="block text-center text-[10px] text-white/70 -mt-4">{val}</span>}
                    </div>
                ))}
            </div>
...
const AlgoTracker: React.FC<{ onPractice: (algoName: string) => void }> = ({ onPractice }) => {
    ...
    const handlePractice = (name: string) => {
        ...
        else alert("Visualizer not available for this specific algorithm yet.");
    };
...
const CodingManager: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'snippets' | 'algorithms' | 'visualizer' | 'bugs'>('visualizer'); // Default to visualizer as requested
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex-grow min-h-0 overflow-hidden">
                ...
                {activeTab === 'visualizer' && <AlgorithmVisualizer initialAlgo={visualizerAlgo} />}
