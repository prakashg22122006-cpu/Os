...
const EditProjectForm: React.FC<{ project: Project; onSave: (p: Project) => void; onCancel: () => void }> = ({ project, onSave, onCancel }) => {
    ...
    return (
        <div className="p-2.5 space-y-2 bg-[rgba(0,0,0,0.2)] rounded-lg border border-dashed border-[#5aa1ff]/30">
    ...
    return (
        <>
            ...
            <div className="mt-2.5 space-y-2">
                {projects.length === 0 ? (
                    <p className="text-[#9fb3cf]">No projects added yet.</p>
                ) : (
                    projects.map((p, i) => (
                        <div key={p.ts}>
                            {editingIndex === i ? (
                                ...
                                <div className="flex justify-between items-start p-2 rounded-lg border border-dashed border-[rgba(255,255,255,0.02)]">
                                    <div>
                                        <strong className="text-white">{p.name}</strong>
                                        <div className="text-sm text-[#9fb3cf]">{p.stack}</div>
                                        <p className="text-sm">{p.desc}</p>
                                    </div>
                                    <div className="flex gap-2 flex-shrink-0 ml-2">
                                        <Button variant="outline" className="px-2 py-1 text-xs" onClick={() => setEditingIndex(i)}>Edit</Button>
                                        <Button variant="outline" className="px-2 py-1 text-xs" onClick={() => deleteProject(i)}>Del</Button>
                                    </div>
                                </div>
                            )}
                        </div>
